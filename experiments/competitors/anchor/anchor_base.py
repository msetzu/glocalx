...
class AnchorBaseBeam(object):
    ...
    def get_sample_fns(sample_fn, tuples, state):
        ...
        def complete_sample_fn(t, n):
            ...
            current_idx = state['current_idx']
            # print('n', 'raw_data.shape', 'data.shape', 'labels.shape', 'current_idx')
            # print(n, raw_data.shape, data.shape, labels.shape, current_idx)
            ...
            try:
                ...
                try:
                    state['labels'][idxs] = labels.squeeze()
                except ValueError:
                    state['labels'][idxs] = labels
                except IndexError as e:
                    # print('\n\n\nError')
                    # print('n', 'raw_data.shape', 'data.shape', 'labels.shape')
                    # print(n, raw_data.shape, data.shape, labels.shape)
                    raise e
            state['current_idx'] += n
                ...
                state['raw_data'] = np.vstack(
                    (state['raw_data'],
                     np.zeros((prealloc_size, raw_data.shape[1]),
                              raw_data.dtype)))
                if len(state['labels'].shape) == 1:
                    try:
                        state['labels'] = np.hstack(
                            (state['labels'][:prealloc_size],
                            np.zeros(prealloc_size, labels.dtype)))
                    except ValueError as e:
                        # print('len(state[labels].shape) == 1')
                        # print(state['labels'][:prealloc_size].shape, np.zeros(prealloc_size, labels.dtype).shape)
                        # print(np.hstack(
                        #     (state['labels'][:prealloc_size],
                        #     np.zeros(prealloc_size, labels.dtype))).shape)
                        raise(e)
                else:
                    try:
                        state['labels'] = np.hstack(
                            (state['labels'].squeeze()[:prealloc_size],
                            np.zeros(prealloc_size, labels.dtype)))
                    except ValueError as e:
                        # print('len(state[labels].shape) != 1')
                        # print(state['labels'].squeeze()[:prealloc_size].shape, np.zeros(prealloc_size, labels.dtype).shape)
                        # print(np.hstack(
                        #     (state['labels'].reshape((-1, 1))[:prealloc_size, :],
                        #     np.zeros(prealloc_size, labels.dtype))).shape)
                        raise(e)
    ...
    def get_anchor_from_tuple(t, state):
        ...
        for f in state['t_order'][t]:
            ...
            covered_true = (
                state['raw_data'][raw_idx][state['labels'][raw_idx].flatten() == 1])
            covered_false = (
                state['raw_data'][raw_idx][state['labels'][raw_idx].flatten() == 0])
            exs = {}
    ...
    def anchor_beam(sample_fn, delta=0.05, epsilon=0.1, batch_size=10,
        ...
        while mean > desired_confidence and lb < desired_confidence - epsilon:
            ...
            try:
                ...
                labels = np.hstack((labels, nlabels.reshape(1, -1)))
            mean = labels.mean()
            lb = AnchorBaseBeam.dlow_bernoulli(mean, beta / data.shape[0])
        if lb > desired_confidence:
            ...
            return anchor
        prealloc_size = batch_size * 100000
        # TODO: Remove
        ...
        raw_data = np.vstack(
            (raw_data, np.zeros((prealloc_size, raw_data.shape[1]),
                                raw_data.dtype)))
        labels = np.hstack((labels, np.zeros(prealloc_size, labels.dtype).reshape(1, -1)))
