...
def learn_local_decision_tree(Z, Yb, weights, class_values, multi_label=False, one_vs_rest=False, cv=3,
                              prune_tree=False):
    dt = DecisionTreeClassifier()
    if prune_tree:
        param_list = {'min_samples_split': [0.01],
        ...
        logger.info('LOREM | Starting grid search...')
        dt_search = GridSearchCV(dt, param_grid=param_list, scoring=scoring, cv=cv, n_jobs=6, iid=False)
        # print(datetime.datetime.now())
        dt_search.fit(Z, Yb, sample_weight=weights)
        logger.info('LOREM | Done with grid search...')
        # print(datetime.datetime.now())
