...
def main():
    ...
    for neigh_type in neigh_list:
        ...
        if black_box in ['DNN', 'DNN0']:
            ...
            def bb_predict_proba(X):
                return bb.predict_proba(X).ravel()
        ...
        explainer = LOREM(K, bb_predict, feature_names, class_name, class_values, numeric_columns, features_map,
                          ...
                          ngen=10, Kc=X_2e, bb_predict_proba=bb_predict_proba)

        filename_neigh = path_neighbors + '%s_%s_%s.json.gz' % (dataset, black_box, neigh_type)
