...
class ImageLimeGenerator(ImageNeighborhoodGenerator):
    ...
    def __init__(self, bb_predict, ocr=0.1, segmentation_type='quickshift'):
        super(ImageLimeGenerator, self).__init__(bb_predict, ocr)
        self.segmentation_type = segmentation_type

        if self.segmentation_type == 'watershed':
            def segmentation_fn(x):
                return watershed(sobel(rgb2gray(x)), markers=128, compactness=0.001)

        elif self.segmentation_type == 'quickshift':
            def segmentation_fn(x):
                return quickshift(x, kernel_size=4, max_dist=200, ratio=0.2)
        else:
            print('unknown segmentation algorithm %s' % self.segmentation_type)
            raise Exception
